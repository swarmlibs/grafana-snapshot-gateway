...
<<EOT
    ...
    for GOARCH in amd64 arm64; do
        export GOARCH
        go build -o /grafana-snapshot-gateway-$GOOS-$GOARCH \
        -ldflags="-s \
...
ARG TARGETOS
ARG TARGETARCH
COPY --from=builder /grafana-snapshot-gateway-$TARGETOS-$TARGETARCH /bin/grafana-snapshot-gateway
ENTRYPOINT [ "/bin/grafana-snapshot-gateway" ]
